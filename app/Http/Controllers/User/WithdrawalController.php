...
class WithdrawalController extends Controller
{
    ...
    public function getotp()
    {
        ...
        // Cleaned up the message string
        $message = "You recently initiated a transfer from your {$settings->site_name} account.\n\n" .
            "Use the code below to complete your transaction:\n\n" .
            "OTP CODE: {$code}\n\n" .
            "If you did not attempt this transaction, please ignore this email or contact support.";

        $subject = "Confirm Your Transaction";

        try {
            // CHANGED: Use to() instead of bcc() for primary delivery
            Mail::to($user->email)->send(new NewNotification($message, $subject, $user->name));
        } catch (\Exception $e) {
            // CHANGED: Log FIRST, then return
            \Log::error('OTP Email Failure: ' . $e->getMessage());
            return redirect()->back()->with('message', 'Email system error. Please try again.');
        }
